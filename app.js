...
function mergeSameCard(id){const c=CARDS.find(x=>x.id===id),gain=nextCard(id);if(!c||!gain)return toast('별이 12 已是最高等級 ✦');if((state.owned[id]||0)<2)return toast('需要兩張相同卡片');state.owned[id]-=2;addCard(gain);save();toast(`合成成功：${c.name} → ${gain.name} ✨`)}
...
function autoMerge(){let did=0;for(let id=1;id<12;id++){while((state.owned[id]||0)>=2){state.owned[id]-=2;addCard(CARDS.find(c=>c.id===id+1));did++}}save();toast(did?`一鍵合成完成：${did} 次 ✨`:'目前沒有兩張相同的可合成卡')}
...
function submitMission(index){const m=state.missions[index];if(!m)return;if((state.owned[m.cardId]||0)<m.qty)return toast(`還需要 ${m.qty-(state.owned[m.cardId]||0)} 張 별이 ${String(m.cardId).padStart(2,'0')}`);state.owned[m.cardId]-=m.qty;state.coins+=m.reward;state.missions[index]=newMission(m.cardId);save();toast(`任務提交成功 +${m.reward.toLocaleString()} ⭐`)}
